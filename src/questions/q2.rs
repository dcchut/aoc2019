...
fn run_interpreter(interpreter: &mut ICInterpreter, noun: i64, verb: i64) -> i64 {
    ...
    interpreter.run(vec![]).get_state(0)
}
...
fn test_interpreter() {
    ...
    assert_eq!(interpreter.run(vec![]).memory, vec![2, 0, 0, 0, 99]);

    let mut interpreter = ICInterpreter::new(vec![2, 3, 0, 3, 99]);
    assert_eq!(interpreter.run(vec![]).memory, vec![2, 3, 0, 6, 99]);

    let mut interpreter = ICInterpreter::new(vec![2, 4, 4, 5, 99, 0]);
    assert_eq!(interpreter.run(vec![]).memory, vec![2, 4, 4, 5, 99, 9801]);

    let mut interpreter = ICInterpreter::new(vec![1, 1, 1, 4, 99, 5, 6, 0, 99]);
    assert_eq!(
        interpreter.run(vec![]).memory,
